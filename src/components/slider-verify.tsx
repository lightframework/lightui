...
import styles from "./slider-verify.less"
...
export { getRandomNumberByRange, square, sum }
...
const SliderVerify = memo(
  ({
    ...
    const [sliderClass, setSliderClass] = useState(styles["sliderContainer"])
    ...
    const reset = () => {
      ...
      // 重置样式
      setSliderLeft(0)
      setSliderClass(styles["sliderContainer"])
    ...
    const handleDragMove = (e: any) => {
      ...
      setSliderClass(
        `${styles["sliderContainer"]} ${styles["sliderContainer_active"]}`,
      )
    ...
    const handleDragEnd = (e: any) => {
      ...
      if (spliced) {
        if (verified) {
          setSliderClass(
            `${styles["sliderContainer"]} ${styles["sliderContainer_success"]}`,
          )
          ...
          setSliderClass(
            `${styles["sliderContainer"]} ${styles["sliderContainer_fail"]}`,
          )
          setTextTip("请再试一次")
          reset()
        }
        ...
        >
          ...
          >
            <div
              ...
              onTouchStart={handleDragStart}
            >
              <div className={styles["sliderIcon"]}>&rarr;</div>
            </div>
          </div>
          <div className={styles["sliderText"]}>{textTip}</div>
        </div>
        <div
