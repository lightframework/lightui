...
/** 同步监控 POST /api/ibex/ctfs/hosts/sync/${param0} */
export async function hostCtfSyncApiIbexCtfsHostsBySyncuid(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: IBEX.hostCtfSyncApiIbexCtfsHostsBySyncuidParams,
  body: IBEX.HostCtfSyncReq,
  options?: { [key: string]: any },
) {
  const { uid: param0, ...queryParams } = params
  return request<IBEX.HostCtfSyncResp>(`/api/ibex/ctfs/hosts/sync/${param0}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
