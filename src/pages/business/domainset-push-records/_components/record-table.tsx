...
import {
  useQueryDomainsetEnvOptions,
  useQueryDomainsetVersionOptions,
} from "@/lib/hooks/data"
import { useToken } from "@/lib/hooks/use-token"
import { tableCellDatetimePostProcess } from "@/lib/utils"
import { domainsetPushRecordsPageListApiOpsDomainsetsPushrecords } from "@/services/ops/domainset"
...
function EnvSelect({
  ...
  const options = useQueryDomainsetEnvOptions()

  return (
    <Select
      value={value}
      options={options.data?.map((item) => ({
        label: item.EnvName ?? item.Uid,
        value: item.Uid,
      }))}
...
function DomainsetSelect({
  value,
  onChange,
}: {
  value?: number
  onChange?: (domainsetId?: number) => void
}) {
  const options = useQueryDomainsetVersionOptions()
...
export default function RecordTable() {
  ...
  const [domainsetId, setDomainsetId] = useState<number | undefined>()

  const [selectedRecordToView, setSelectedRecordToView] = useState<
    OPS.DomainsetPushRecord | undefined
  >()
  ...
  const columns: TableColumns<OPS.DomainsetPushRecord> = [
    {
      title: "ID",
      dataIndex: "id",
      width: 80,
    },
    {
      ...
      width: 200,
      copyable: true,
      render: access.domainsetPushRecordsReadOneApiOpsDomainsetsByPushrecordsid
        ? (_, row) => (
            <Paragraph
          ...
          actions={[
            {
              ...
              disabled:
                row.pushType === "back" ||
                !access.domainsetBackApiOpsDomainsetsBack,
            },
          ]}
        />
      ),
  ...
  return (
    <>
      <Table
        ...
        request={domainsetPushRecordsPageListApiOpsDomainsetsPushrecords}
        toolbar={{
          subTitle: (
            <div className="flex items-center gap-2">
              <EnvSelect onChange={setEnvUid} />
              <DomainsetSelect onChange={setDomainsetId} />
            </div>
          ),
        }}
